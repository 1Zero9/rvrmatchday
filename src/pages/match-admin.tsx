...
export default function MatchAdminNew() {
  ...
  // Quick Teams View filters
  const [teamTypeFilter, setTeamTypeFilter] = useState<'all' | 'rvr' | 'opponent'>('all');
  ...
  // Filter teams for Quick Teams View
  const getFilteredTeams = () => {
    let filtered = teams;

    // Filter by team type
    if (teamTypeFilter === 'rvr') {
      filtered = filtered.filter(team => !team.isOpponent);
    } else if (teamTypeFilter === 'opponent') {
      filtered = filtered.filter(team => team.isOpponent);
    }
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="text-center mb-6 relative">
            <div className="inline-flex items-center gap-3 mb-2">
              <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center text-white text-lg shadow-lg">
                ⚽
              </div>
              <h1 className="text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">Team Setup</h1>
            </div>
            <p className="text-sm text-gray-500">Create and manage your teams</p>
            <Link 
              href="/admin" 
              className="absolute top-0 right-0 text-gray-400 hover:text-gray-600 text-sm flex items-center space-x-1 bg-white px-4 py-2 rounded-full shadow-sm border border-gray-200 hover:shadow-md transition-all"
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md border border-white/50 p-4 mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-gray-800">
                Step {getStepNumber(currentStep)} of {getTotalSteps()}
              </span>
              <span className="text-sm text-gray-600">{stepNames[currentStep]}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div 
                className="h-3 rounded-full transition-all duration-300 bg-gradient-to-r from-green-500 to-emerald-600 shadow-sm"
          ...
          <AnimatePresence mode="wait">
            ...
            {currentStep === 'type' && (
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className="bg-white/90 backdrop-blur-sm rounded-lg shadow-lg border border-white/50 p-6"
              >
                ...
                <div className="grid grid-cols-2 gap-3 max-w-lg mx-auto">
                  ...
                  >
                    <div className="text-center">
                      ...
                      <h3 className="text-sm font-bold text-orange-700 mb-1">Opponent Team</h3>
                      <p className="text-sm text-gray-600">External team</p>
                    </div>
                  </button>
                </div>
                <div className="flex justify-center mt-3">
                  <button
                    ...
                    className="px-4 py-1 bg-white hover:bg-gray-50 text-gray-600 hover:text-gray-800 rounded-md border border-gray-200 hover:border-gray-300 font-medium transition-all duration-200 shadow-sm hover:shadow-md text-sm"
                  >
                    ← Cancel
                  </button>
                </div>
              ...
              >
                ...
                <div className="max-w-xl mx-auto space-y-2">
                  ...
                  <div className="bg-blue-50 rounded-lg p-2 border border-blue-200">
                    <label className="flex items-center gap-1 text-sm font-bold text-blue-800 mb-1">
                      ⚽ Team Name *
                      {validationErrors.teamName && <span className="text-red-500 ml-1 text-sm">({validationErrors.teamName})</span>}
                    </label>
                    <input
                      type="text"
                      ...
                      className={`w-full px-2 py-1.5 text-sm border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500/20 transition-all duration-200 bg-white ${
                  ...
                  <div className="bg-green-50 rounded-lg p-2 border border-green-200">
                    <label className="flex items-center gap-1 text-sm font-bold text-green-800 mb-1">
                      🏆 Age Group *
                      {validationErrors.ageGroup && <span className="text-red-500 ml-1 text-sm">({validationErrors.ageGroup})</span>}
                    </label>
                    <div className="relative">
                      <select
                        ...
                        className={`w-full px-2 py-1.5 text-sm border rounded-md focus:outline-none focus:ring-1 focus:ring-green-500/20 bg-white ${
                    ...
                    <p className="text-sm text-green-600 mt-1">
                      {ageGroups.length} groups available
                    </p>
                  </div>
                  ...
                  <div className="bg-purple-50 rounded-lg p-2 border border-purple-200">
                    ...
                    <div className="grid grid-cols-3 gap-1">
                      {[{value: 'Male', icon: '♂️', color: 'blue'}, {value: 'Female', icon: '♀️', color: 'pink'}, {value: 'Mixed', icon: '🤝', color: 'purple'}].map(option => (
                        <button
                          key={option.value}
                          type="button"
                          onClick={() => setWizardData(prev => ({ ...prev, gender: option.value as any }))}
                          className={`py-1 px-1 rounded-md border transition-all duration-200 ${
                            wizardData.gender === option.value
                              ? `border-${option.color}-500 bg-${option.color}-100 shadow-sm`
                              : `border-gray-200 bg-white hover:border-${option.color}-300 hover:bg-${option.color}-50`
                          }`}
                        >
                          <div className="text-center">
                            <div className="text-sm mb-0.5">{option.icon}</div>
                            <div className="text-sm font-semibold text-gray-700">{option.value}</div>
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                <div className="flex justify-center items-center mt-6 gap-4">
                  <button
                    type="button"
                    onClick={prevStep}
                    className="px-4 py-2 bg-white hover:bg-gray-50 text-gray-600 hover:text-gray-800 rounded-md border border-gray-200 hover:border-gray-300 font-medium transition-all duration-200 text-sm"
                  >
                    ← Back
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className={`px-6 py-2 disabled:bg-gray-300 disabled:cursor-not-allowed disabled:text-gray-500 text-white rounded-md font-bold transition-all duration-200 text-sm ${
                      wizardData.teamType === 'rvr' ? 'bg-green-600 hover:bg-green-700' : 'bg-orange-600 hover:bg-orange-700'
                    }`}
                  >
                    Continue →
                  </button>
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="bg-blue-50 rounded-lg p-4 border border-blue-200 relative">
                    ...
                    >
                      ...
                      <option value="Friendly Only">Friendly Only</option>
                      {leagues.map(league => (
                  ...
                  <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 text-sm border border-purple-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                    >
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <input
                        ...
                        className="px-3 py-2 text-sm border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-gray-500"
                      />
                      <input
                        type="tel"
                        placeholder="Phone"
                ...
                <div className="flex justify-center items-center mt-6 gap-3">
                  ...
                  <button
                    ...
                    className="px-6 py-2 disabled:bg-gray-300 disabled:cursor-not-allowed disabled:text-gray-500 text-white rounded-md font-bold transition-all duration-200 text-sm bg-green-600 hover:bg-green-700"
                  >
                    Continue →
                  </button>
                </div>
              ...
              >
                <div className="text-center mb-6">
                  <div className="inline-flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg flex items-center justify-center text-white text-lg shadow-lg">👨‍🏫</div>
                    <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">
                      Team Coaches
                    </h2>
                  </div>
                  <p className="text-sm text-gray-600">Select and manage team coaching staff</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          ...
                          <button
                            ...
                            className="w-full py-2 bg-green-100 hover:bg-green-200 text-green-700 rounded-md transition-colors border border-dashed border-green-300 text-sm font-medium"
                          >
                            + Add Coach
                          </button>
                        </div>
                      ...
                      <div className="text-sm text-green-600 bg-green-100/50 p-3 rounded-md border border-green-200">
                        💡 Selected coaches appear on right →
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                    <label className="flex items-center gap-2 text-sm font-semibold text-blue-800 mb-3">
                      👨‍🏫 Selected ({wizardData.coaches.length})
                    </label>
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {wizardData.coaches.length === 0 ? (
                        <div className="text-center py-6">
                          ...
                          <p className="text-blue-600 text-sm">None selected</p>
                        </div>
                      ) : (
                        wizardData.coaches.map((coach, index) => (
                          <div key={index} className="flex justify-between items-center bg-white p-3 rounded-md shadow-sm border border-blue-200">
                            <span className="text-sm font-medium text-blue-800">{coach}</span>
                            <button
                              onClick={() => {
                                setWizardData(prev => ({
                                  ...prev,
                              ...
                              className="text-red-600 hover:text-red-800 transition-colors px-2 py-1 hover:bg-red-50 rounded text-sm"
                              title="Remove coach"
                            >
                              ✕
                            </button>
                ...
                <div className="flex justify-center items-center mt-6 gap-4">
                  ...
                  <button
                    type="button"
                    onClick={nextStep}
                    className={`px-6 py-2 text-white rounded-md font-bold transition-all duration-200 text-sm ${wizardColors.background} hover:opacity-90`}
                  >
                    Continue →
                  </button>
                </div>
            ...
            {currentStep === 'squad' && wizardData.teamType === 'rvr' && (
              ...
              >
                <div className="text-center mb-6">
                  <div className="inline-flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-600 rounded-lg flex items-center justify-center text-white text-lg shadow-lg">👥</div>
                    <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">
                      Team Squad
                    </h2>
                  </div>
                  ...
                  <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                          <div className="space-y-1.5">
                            ...
                            <button
                              ...
                              className="w-full py-1.5 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-md transition-colors border border-dashed border-purple-300 text-sm font-medium"
                            >
                              + Add Player
                            </button>
                          </div>
                          
                          <div className="text-sm text-purple-600 bg-purple-100/50 p-1.5 rounded-md border border-purple-200">
                            💡 Players appear on right →
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="border-t border-purple-200 pt-1.5">
                        <label className="text-sm font-bold text-purple-700 mb-1 flex items-center gap-1">
                          🔗 Duplicates
                        </label>
                        <button
                          onClick={() => {
                          ...
                          className="w-full py-1.5 bg-yellow-100 hover:bg-yellow-200 text-yellow-700 rounded-md transition-colors text-sm font-medium"
                        >
                          🔍 Check
                        </button>
                      </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 mb-6">
                    <h3 className="text-xl font-bold mb-4 flex items-center">
                      {wizardData.teamType === 'rvr' ? (
                        ...
                        <><span className="text-base mr-2">🏟️</span>Opponent Team</>
                      )}
                    </h3>
          ...
          {currentStep !== 'complete' && teams.length > 0 && (
            <div className="mt-8 bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="mb-6 space-y-4">
                {/* Team Type Filters */}
                <div>
                  <p className="text-sm font-semibold text-gray-700 mb-2">Team Type</p>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      onClick={() => setTeamTypeFilter('opponent')}
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                        teamTypeFilter === 'opponent'
                ...
                {uniqueLeagues.length > 1 && (
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-2">League</p>
                    <div className="flex flex-wrap gap-2">
                      <button
              ...
              {filteredTeams.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredTeams.slice(0, 9).map(team => (
                    <div key={team.id} className="border border-gray-200 rounded-lg p-3 hover:shadow-md transition-shadow cursor-pointer"
                         onClick={() => router.push(`/match-admin?edit=${team.id}`)}>
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <h4 className="font-medium text-gray-900">{team.name}</h4>
                          <p className="text-sm text-gray-600">{team.ageGroup} • {team.league}</p>
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          <button 
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              router.push(`/match-admin?edit=${team.id}`);
                            }}
                          >
                            ✏️ Edit
                          </button>
                ...
                <div className="text-center py-8">
                  <div className="text-4xl mb-2">🔍</div>
                  <h4 className="font-medium text-gray-900 mb-1">No teams found</h4>
                  <p className="text-sm text-gray-600">Try adjusting your filters or create a new team above</p>
                </div>
              )}

              {filteredTeams.length > 9 && (
