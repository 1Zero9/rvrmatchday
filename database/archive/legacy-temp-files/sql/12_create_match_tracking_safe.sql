-- Safe Match Tracking Database Schema - Drop and Recreate
-- Run this to safely create all required tables for match tracking
...
-- 1. Create teams table with ALL columns
CREATE TABLE teams (
...
-- 2. Create players table with ALL columns
...
-- 3. Create matches table with ALL columns
...
-- 4. Create match_events table with ALL columns
...
-- 5. Now create ALL indexes (after all tables and columns exist)
CREATE INDEX idx_teams_active ON teams(is_active);
CREATE INDEX idx_teams_opponent ON teams(is_opponent);
